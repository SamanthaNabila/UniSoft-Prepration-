...
def test_search_handles_corrupt_json_file_gracefully(tmp_path, capsys):
    notes_file = tmp_path / "notes.json"
    notes_file.write_text("{not valid json")
...
def test_add_handles_corrupt_json_file_gracefully(tmp_path, capsys):
    notes_file = tmp_path / "notes.json"
    notes_file.write_text("{not valid json")

    with pytest.raises(SystemExit) as error:
        main(["add", "Title", "Body", "--file", str(notes_file)])

    assert error.value.code == 1
    assert "Invalid notes file" in capsys.readouterr().err


def test_delete_handles_corrupt_json_file_gracefully(tmp_path, monkeypatch, capsys):
    notes_file = tmp_path / "notes.json"
    notes_file.write_text("{not valid json")
    monkeypatch.setattr("builtins.input", lambda prompt: "yes")

    with pytest.raises(SystemExit) as error:
        main(["delete", "Title", "--file", str(notes_file)])

    assert error.value.code == 1
    assert "Invalid notes file" in capsys.readouterr().err


def test_delete_handles_read_only_notes_file_gracefully(tmp_path, monkeypatch, capsys):
    notes_file = tmp_path / "notes.json"
...
def test_delete_prints_message_when_note_not_found(tmp_path, monkeypatch, capsys):
    notes_file = tmp_path / "notes.json"
    monkeypatch.setattr("builtins.input", lambda prompt: "yes")

    main(["delete", "Missing", "--file", str(notes_file)])

    assert capsys.readouterr().out == "Note not found: Missing\n"
    assert not notes_file.exists()
