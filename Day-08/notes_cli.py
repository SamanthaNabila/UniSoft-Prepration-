...
def _load_notes_or_exit(path):
    try:
        return load_notes(path)
    except json.JSONDecodeError:
        print(f"Invalid notes file: {path}", file=sys.stderr)
        raise SystemExit(1)


def _save_notes_or_exit(path, notes):
    try:
        save_notes(path, notes)
    except PermissionError:
        print(f"Cannot write notes file: {path}", file=sys.stderr)
        raise SystemExit(1)
...
def main(argv=None):
    ...
    if args.command == "add":
        ...
        notes = _load_notes_or_exit(args.file)
        notes.append({"title": args.title, "body": args.body})
        _save_notes_or_exit(args.file, notes)
        return
    ...
    if args.command == "search":
        ...
        for note in _load_notes_or_exit(args.file):
        ...
        for note_index, note in enumerate(notes):
            if note["title"] == args.title:
                ...
                if confirmation.casefold() in ("y", "yes"):
                    _save_notes_or_exit(args.file, notes[:note_index] + notes[note_index + 1 :])
                return
